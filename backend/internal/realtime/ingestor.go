...
type Ingestor struct {
	...
	onTriggerAI      func(models.ActivityIndex)
...
// SetAnalysisTrigger binds the Gemini AI decision analysis callback
...
func (in *Ingestor) Start(ctx context.Context) {
	...
	go func() {
		...
		in.mu.RLock()
		trigger := in.onTriggerAI
		in.mu.RUnlock()
		if trigger != nil {
			log.Println("[INFO] Triggering initial Gemini AI baseline assessment...")
...
func (in *Ingestor) processRealEarthquake(evt models.SeismicEvent, source string, potensi string) {
	...
	if evt.Magnitude >= 4.8 && aiTrigger != nil {
		go aiTrigger(actIdx)
	}
}
